...
from seedener.helpers.ur2.ur_decoder import URDecoder
...
class DecodeQR:
    ...
    def add_data(self, data):
        ...
        if self.qr_type == None:
            ...
            if self.qr_type in [QRType.PSBT__UR2, QRType.OUTPUT__UR, QRType.ACCOUNT__UR, QRType.BYTES__UR]:
                ...
                self.decoder = ChiaAddressQrDecoder() # Single Segment chia address
    ...
    @property
    def is_address(self):
        return self.qr_type == QRType.CHIA_ADDRESS
    ...
    def detect_segment_type(s, wordlist_language_code=None):
        ...
        try:
            ...
            # Chia Address
            elif DecodeQR.is_chia_address(s):
                return QRType.CHIA_ADDRESS
    ...
    @staticmethod
    def is_chia_address(s):
        if re.search(r'^chia\:.*', s, re.IGNORECASE):
            return True
...
class SeedQrDecoder(BaseSingleFrameQrDecoder):
    """
        ...
        Supports seedener SeedQR numeric (wordlist indices) representation of a seed.
        Supports seedener CompactSeedQR entropy byte representation of a seed.
...
class ChiaAddressQrDecoder(BaseSingleFrameQrDecoder):
    """
        Decodes single frame representing a chia address
    """
    def __init__(self):
        super().__init__()
        self.address = None
    ...
    def add(self, segment, qr_type=QRType.CHIA_ADDRESS):
