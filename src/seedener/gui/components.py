...
class SeedenerCustomIconConstants:
    LARGE_CHEVRON_LEFT = "\ue900"
    SMALL_CHEVRON_RIGHT = "\ue901"
    PAGE_UP = "\ue903"
    PAGE_DOWN = "\ue902"
...
class Icon(BaseComponent):
    screen_x: int = 0
    screen_y: int = 0
    icon_name: str = SeedenerCustomIconConstants.CHIA_LOGO_TILTED
    ...
    def __post_init__(self):
        ...
        if SeedenerCustomIconConstants.MIN_VALUE <= self.icon_name and self.icon_name <= SeedenerCustomIconConstants.MAX_VALUE:
            self.icon_font = Fonts.get_font(GUIConstants.ICON_FONT_NAME__SEEDENER, self.icon_size, file_extension="otf")
        else:
...
class FormattedAddress(BaseComponent):
    """
        Display a CHIA address in a "{first 7} {middle} {last 7}" formatted view with
...
class BtcAmount(BaseComponent):
    ...
    def __post_init__(self):
        ...
        # Render the circular CHIA icon
        btc_icon = Icon(
            image_draw=draw,
            canvas=self.paste_image,
            icon_name=SeedenerCustomIconConstants.CHIA_LOGO_TILTED,
    ...
    def __post_init__(self):
        ...
        if self.show_back_button:
            self.left_button = IconButton(
                icon_name=SeedenerCustomIconConstants.LARGE_CHEVRON_LEFT,
